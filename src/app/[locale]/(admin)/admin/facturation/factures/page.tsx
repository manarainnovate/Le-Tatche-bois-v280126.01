...
// ═══════════════════════════════════════════════════════════
// Server Component - Factures List Page
// ═══════════════════════════════════════════════════════════
...
export default async function FacturesPage({ params, searchParams }: FacturesPageProps) {
  ...
  const [documents, total, clients] = await Promise.all([
    prisma.cRMDocument.findMany({
      ...
      include: {
        ...
        _count: {
          select: { items: true, payments: true, children: true },
        },
      },
  ...
  // Stats
  const [stats, overdueStats] = await Promise.all([
    ...
    >
      <FacturesPageClient
        ...
        stats={stats.map((s) => ({
          ...
          paid: s._sum.paidAmount ? Number(s._sum.paidAmount) : 0,
          balance: s._sum.balance ? Number(s._sum.balance) : 0,
        }))}
        overdueStats={{
          count: overdueStats._count || 0,
          balance: overdueStats._sum.balance ? Number(overdueStats._sum.balance) : 0,
        }}
      />
    </Suspense>
  );
