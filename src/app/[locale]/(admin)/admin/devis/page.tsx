...
// ═══════════════════════════════════════════════════════════
// Server Component - Devis List Page
// ═══════════════════════════════════════════════════════════
...
export default async function DevisPage({ params, searchParams }: DevisPageProps) {
  ...
  // Fetch documents
  const [documents, total, clients, projects] = await Promise.all([
  ...
  const transformedDocuments = documents.map((doc) => ({
    ...doc,
    client: doc.client ? { ...doc.client, name: doc.client.fullName } : undefined,
  ...
  return (
    ...
    >
      <DevisPageClient
        ...
        clients={clients.map((c) => ({ ...c, name: c.fullName }))}
        projects={projects}
        locale={locale}
        currentPage={page}
